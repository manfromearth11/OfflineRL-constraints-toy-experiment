"""Lambda sweep for OT methods in multimodal offline RL.

Runs:
  - OT methods with explicit lambda sweep
...
def main():
    parser = argparse.ArgumentParser(description="Sweep OT lambda in multimodal offline RL.")
    ...
    parser.add_argument("--ot-methods", type=str, default="wasserstein,partial_ot,unbalanced_ot")
    ...
    save_csv(
        raw_csv,
        raw_records,
        fieldnames=["seed", "method", "method_label", "lambda", "mean_reward", "final_loss", "is_baseline"],
    )
    save_csv(
        summary_csv,
        summary,
        fieldnames=["method", "lambda", "n_seeds", "mean_reward_mean", "mean_reward_std", "mean_reward_stderr"],
    )
