"""Dimension sweep toy: show KL failure vs OT robustness under Q misranking.
...
def true_reward_torch(a: torch.Tensor) -> torch.Tensor:
    return -a.pow(2).sum(dim=-1)
...
def evaluate_metrics(
    ...
    with torch.no_grad():
        ...
        reward_mean = float(true_reward_torch(a).mean().item())
...
def ot_plan_cost(
    ...
    elif method == "partial_ot":
        T = pot.partial.partial_wasserstein(uniform, uniform, C_np, m=partial_mass)
...
def train_ot(
    ...
    partial_mass = float(ot_cfg["partial_mass"])
    ...
    for _ in range(epochs):
        for _ in range(steps):
            ...
            a_b_np, _ = sample_behavior(bsz, means_np, stds_np, weights_np, rng)
...
def plot_reward_curves(summary: List[dict], lambdas: List[float], out_path: str):
    ...
    for ax, dim in zip(axes, dims):
        ...
        for method, color in [("wasserstein", "tab:blue"), ("partial_ot", "tab:green"), ("unbalanced_ot", "tab:orange")]:
...
def build_winner_table(summary: List[dict]) -> List[dict]:
    ...
    for dim in dims:
        ...
        best = {}
        for m in ["wasserstein", "partial_ot", "unbalanced_ot"]:
...
def main():
    ...
    print("=" * 88)
    print("OT-WINS DIMENSION SWEEP TOY")
    ...
    for dim in dims:
        ...
        for seed in seeds:
            ...
            a_np, _ = sample_behavior(int(env_cfg["n_data"]), means, stds, weights, rng)
            actions_t = torch.tensor(a_np, dtype=torch.float32)

            # Dataset-side fixed corrupted Q for KL weighting
            ...
            for lam in lambdas:
                for method in ["wasserstein", "partial_ot", "unbalanced_ot"]:
                    ...
                    loss = train_ot(
                        ...
                        method=method,
                        lam=float(lam),
                        seed=seed * 1000 + int(round(lam * 1000)) + (1 if method == "wasserstein" else (2 if method == "partial_ot" else 3)),
                    )
                    m = evaluate_metrics(
                        p,
                        means,
    ...
    plot_reward_curves(summary, lambdas, reward_png)
