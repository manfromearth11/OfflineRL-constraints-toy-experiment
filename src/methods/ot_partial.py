...
class PartialOTPolicy(OfflineRLMethod):
    """Partial OT: transport only top-m fraction of mass."""

    def __init__(self, policy: GaussianPolicy, config: dict, env_config: dict, q_fn=None):
        super().__init__(policy, config)
        self.lam = config.get("lam", 1.0)
        self.mass_fraction = config.get("mass_fraction", 0.5)
    ...
    @property
    def name(self) -> str:
        return f"Partial OT (m={self.mass_fraction})"
    ...
    def compute_loss(self, batch: dict) -> torch.Tensor:
        ...
        pot_loss = torch.tensor(0.0, device=device)
        for i in range(B_ot):
            C = torch.cdist(a_policy[i], a_behavior[i], p=2) ** 2
            C_np = C.detach().cpu().numpy().astype(np.float64)
            T_partial = pot.partial.partial_wasserstein(
                uniform, uniform, C_np, m=self.mass_fraction
            )
            T_tensor = torch.tensor(T_partial, dtype=torch.float32, device=device)
            pot_loss = pot_loss + (T_tensor * C).sum()
        pot_loss = pot_loss / B_ot

        return -q_vals + self.lam * pot_loss
